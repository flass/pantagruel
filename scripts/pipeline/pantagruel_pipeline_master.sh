...
usagelong (){
  ...
  echo "                        (available for tasks: 00-08)"
  ...
  echo "                        Note 2: this does not apply to tasks run through HPC scripts,"
  echo "                          which have their own interface to define numbers of CPUs to be used."
  echo ""
  ...
  echo "                        (for the moment only available for tasks: 00, 02 i.e. 'fetch', 'align')"
  echo ""
  ...
  echo "                           to change the value of an already set option, e.g. switch to use GeneRax reconciliation method:"
  echo "                             \`pantagruel -i config_file --refresh -e GeneRax init\`"
  echo "                           to both turn on collapsing and switch to use GeneRax reconciliation method:"
  echo "                             \`pantagruel -i config_file --refresh -c -e GeneRax init\`"
  echo "                        Currently, only short options (e.g., -e or -n, NOT --rec_method or --collapse) are supported after --refresh."
  echo ""
  ...
  echo "                        - an 'annotation/' folder, where are stored annotation folders, one per genome. Each genome annotation file set"
  echo "                           must be stored in a separate folder, which name must match a contig file (e.g. 'seqProjID/' for 'seqProjID.fasta')."
  echo "                           Alternatively, each folder can be provided as a .tar or .tar.gz archive containing the annotation folder"
  echo "                           (e.g. 'seqProjID.tar' or 'seqProjID.tar.gz', containing the folder 'seqProjID/')."
  echo "                             each genome annotation folder should contain:"
  ...
  echo "                          If at least one of the .gbk, .ffn or .faa are missing, all three will be derived from the .gff source."
  echo "                          NOTE: These four files are typically produced when using Prokka for annotation; thus to ensure proper parsing,"
  echo "                                it is strongly advised that any provided annotation was generated with Prokka"
  ...
  echo ""
  echo "    -S|--snp_aln        reduce the core-genome alignment to SNPs"
  echo ""
