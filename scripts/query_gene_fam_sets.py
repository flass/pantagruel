...
def main(dbconpar, baseq, minsizes=[], maxsizes=[], dirout='', outprefix=''):
	...
	dbcon = psycopg2.connect(dbconpar)
	dbcur = dbcon.cursor()
...
def usage():
	s = 'Usage: python %s --db.con.par="dbname=abcdb user=efgh" [ --famsets.min.sizes="[x][,y,...,]z" --famsets.max.sizes=X[,Y,...,][Z] ] [OPTIONS]'%(sys.argv[0])
	return s
...
if __name__=='__main__':	
	...
	main(dbconpar, baseq=basequery, minsizes=minsizes, maxsizes=maxsizes, dirout=dirout, outprefix=outprefix)
