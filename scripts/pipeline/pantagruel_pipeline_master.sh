...
usage (){
  ...
  echo "                      _mandatory_ "
  echo "                       - a 'contigs/' folder, where are stored multi-FASTA files of genome assembly (one file per genome),"
  echo "                         which name (truncated by removing the '.fa' string and everything occuring after) will be further"
  echo "                         kept as the assembly_id (beware of names redundant with RefSeq assemblies)."
  ...
  echo "                      _optional_ "
  echo "                       - an 'annotation/' folder, where are stored .gff and .gbk files (GFF 3 and GenBank flat file formats),"
  echo "                         ideally resulting from annotation with Prokka. Each genome annotation file set must be stored in"
  echo "                         a separate folder, which name must match a contig file (e.g. 'assembId/' for 'assembId.fasta')."
  echo ""
