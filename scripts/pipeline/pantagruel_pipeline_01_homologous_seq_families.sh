...
if [ ! -z "${updatedbfrom}" ] ; then
  ...
  prevdbprotali=${updatedbfrom}/$(basename ${protali})
  if [ ! -d "${prevdbprotali}" ] ; then
    ...
    prevdballfaarad=${prevdbprotali}/$(basename ${allfaarad})
...
if [ -z "${updatedbfrom}" ] ; then
	...
	# update previous clustering
	prevdbmmseqsclout=${prevdbprotali}/$(basename ${families})/$(basename ${allfaarad}.nr).mmseqs_clusterdb_default
