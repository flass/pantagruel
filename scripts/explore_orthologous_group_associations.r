...
parseOGMatchFile = function(nfogmatch, quant.only=F, nmaxrlocsid=NULL, refrepliordlineages=NULL,
...
#~ 	matchev = unique(read.table(nfogmatch))
	matchev = read.table(nfogmatch, sep='\t', header=F, colClasses="numeric")
	...
	if (comp.quant){
		...
		if (plot.quant){
			plotCoEvolQuantiles(qmatchev, detail.quant, sample.name=basename(nfogmatch))
		}
	}else{
		qmatchev = NULL
	}
	...
	if (verbose){ print(sprintf("%s ... done. Top co-evolution scores (%s):", nfogmatch, topstr), quote=F) ; print(head(topmatchev)) ; print(sprintf("... (%d lines)", nrow(topmatchev))) }
...
spec = matrix(c(
  'dir_match_events',   'i', 1, "character", "[mandatory] (preferably absolute) path of file listing associated ortholog groups (output of condense_coevolution_network_by_orthologous_groups.py)",
...
patnfogmatch = file.path(dirmatchevents, 'matching_events.tab*')
#~ print(patnfogmatch)
lnfogmatch = Sys.glob(patnfogmatch)
if (!is.null(opt$parse_max_files)){
	lnfogmatch = lnfogmatch[1:opt$parse_max_files]
}
#~ print(head(lnfogmatch))
if (length(lnfogmatch)>1){
	lnfogmatch = lnfogmatch[order(as.numeric((sapply(strsplit(lnfogmatch, split='\\.'), function(x){ x[length(x)] }))))]
	print(sprintf("will load lineage co-evolution scores from %d files matchin '%s'", length(lnfogmatch), patnfogmatch), quote=F)
}
...
if (!is.null(qcutoff)){
	...
	if ((!is.null(opt$load_quantiles) | loadup>=1) & file.exists(nfsavematchev)){
		...
		lparsematchev = mclapply(lnfogmatch, parseMatchEventFile, quant.only=T, top.quant.cutoff=qcutoff, verbose=verbose, mc.cores=ncores)
		names(lparsematchev) = basename(lnfogmatch)
...
#~ 	save(matchevdf, totrepcomp, avg.cutoff.val, file=paste(file.path(dirout, prefixout), 'co-evolution_quantiles.RData', sep='.'))
	...
	print(sprintf("found an average value of %f for quantiles at p=%f of empircal distribution of co-evolution scores over %g reported gene lineage comparisons (stored within %d separate files)",
				   avg.cutoff.val, qcutoff, totrepcomp, length(lnfogmatch)), quote=F)
...
if ((!is.null(opt$load_top_ass) | loadup>=2) & file.exists(nftopasstab) & is.null(refrepliordlineages)){
	...
	if (verbose) print(sprintf("loaded top association data from file: '%s'", nftopass))
		...
		lparsematchev = mclapply(lnfogmatch, parseMatchEventFile, comp.quant=F, top.val.cutoff=scutoff, 
